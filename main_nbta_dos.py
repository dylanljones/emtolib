...
def generate_figure(datasets):
    ...
    print(" ".join(str(ds.attrs["dos_ef"] / (1.41 * ry_to_ev)) for ds in [dsv, ds25, ds50, ds75, dsnb]))
    ...
    ax2.plot(dsv.energy, dsv.dos, label="Ta", lw=lw)
    ax3.plot(ds25.energy, ds25.dos, label=r"$x=0.25$", lw=lw)
    ax3.plot(ds50.energy, ds50.dos, label=r"$x=0.50$", lw=lw)
    ax3.plot(ds75.energy, ds75.dos, label=r"$x=0.75$", lw=lw)
    ...
    fig.savefig(FIGDIR / "NbTa_dos_cpa.png")
...
def main():
    ...
    generate_figure(datasets)
    plt.show()

    return

    fig, ax = plt.subplots()
    for c_nb, ds in datasets.items():
        ax.plot(ds.energy, ds.dos, label=f"{c_nb:.2f}")
    ax.legend()
    plt.show()
